...
\end{frame}
\begin{frame}
	\frametitle{Mean Field Inference In Hidden CRF }
	\begin{algorithm}[H]
		\caption{Mean-field inference in fully-connected CRF}
		\KwIn{$Q_i(h_i) \gets U_i(h_i), \; i = 1, 2, \dots, N$ \tcp*[f]{Initialize $Q(h)$}}
		\While{not reach max iteration number}{
		    $\hat{Q}_i^{(m)}(h_i) \gets \sum_{j \neq i} k^{(m)}(f_i, f_j) Q_j(h_i)$ for all $m$ \tcp*[f]{Message Passing}\;
		    $\tilde{Q}_i^{(m)}(h_i) \gets \sum_{l \in \mathcal{L}} \mu^{(m)}(h_i, l) \sum_m \omega^{(m)} \hat{Q}_i^{(m)}(l)$ \tcp*[f]{Compatibility Transform}\;
		    $Q_i(h_i) \gets \exp \{\phi(h_j, x_j; \omega) + \varphi(y, h_j, x_j; \delta) - \tilde{Q}_i(x_i)\}$ \tcp*[f]{Local Update}\;
		    Normalize $Q_i(h_i)$\;
		}
		\end{algorithm}
\end{frame}
